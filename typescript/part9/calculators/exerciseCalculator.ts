...
export const parseArgsArr = (args: string[]): exerciseDataType => {
  if (args.length < 4) {
    throw new Error(
      'incorrect input. try: npm run bmiCalculator.ts target data1, ...'
    );
  }
...
export const calculateExercises = (data: number[], target: number): exerciseType => {
  const average =
